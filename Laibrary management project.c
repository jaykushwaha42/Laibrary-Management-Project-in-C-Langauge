...
void mainmenu()
{
	...
	case '4':
		...
		printf("Programmers....");
		gotoxy(16,4);
		printf("1. Dheeraj kumar       (BCA-I)");
		gotoxy(16,5);
		printf("   Mobile:9199901992  E-mail:welldheeraj@hotmail.com");
		gotoxy(16,7);
		printf("2. Navneet kumar jha     (BCA-I)");
		gotoxy(16,8);
		printf("   Mobile:8002121674  E-mail:navneetjha39@zoho.com");
	
	
		
		gotoxy(16,13);
		printf("With  the Unexplainable Help of Er. Vikash kumar\n\t\tMob:-7352585586\n\t\tE-mail:-vikash6955.kumar@gmail.com");
		gotoxy(10,17);
...
void addbooks(void)    //funtion that add books
{
	...
	gotoxy(20,11);
	printf("\xDB\xDB\xDB\xDB\xB2 3. Robotics");
	gotoxy(20,13);
	printf("\xDB\xDB\xDB\xDB\xB2 4. Hacking");
	gotoxy(20,15);
...
void deletebooks()    //function that delete items from file fp
{
	...
	if(findbook=='t' )
	{
	    ...
	    if(getch()=='y')
	    {
		ft=fopen("nav.dat","wb+");  //temporary file for delete
		rewind(fp);
		while(fread(&a,sizeof(a),1,fp)==1)
		{
		    if(a.id!=d)
		...
		fclose(fp);
		remove("dheeraj.dat");
		rename("nav.dat","dheeraj.dat"); //copy all item from temporary file to fp except that
		fp=fopen("dheeraj.dat","rb+");    //we want to delete
		if(findbook=='t')
		{
		    gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("dheeraj.dat","rb+"); //open file for reading propose
...
void Password(void) //for password option
{
   ...
   pass[i] = '\0';
   if(strcmp(pass,psd)==0||strcmp(pass,password)==0)
   {

	gotoxy(15,9);
	//textcolor(BLINK);
	printf("Your Password Is Matching");
	for(j=0;j<20;j++)
	{
		delay(50);
		printf("*");
	...
	//printf("");
	gotoxy(17,10);
	printf("\n\n\t\t\xB2\xB2\xB2Password is matched\xB2\xB2\xB2\n\n\t\tPress any key to countinue.....");
	getch();
	mainmenu();
   }
   else
